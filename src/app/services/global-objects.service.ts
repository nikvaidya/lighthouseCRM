...
import { LoadingController, ToastController, Platform, Events, AlertController, ModalController } from '@ionic/angular';
...
export class GlobalObjectsService {
  ...
  public android_version_numeric = 29;
  ...
  // parentAppkey = "RCPLSR";
  // parentAppkey = "LHSCARE";
  // parentAppkey = "KEPROCRMSE";
  // parentAppkey = "JSPLCRMSE"
  // parentAppkey = "RSONVMA";
  // parentAppkey = "KEPROCRMSE";
  // parentAppkey = "MPPLAM";
  // parentAppkey = "MSPLCERP"
  // parentAppkey = "JSPLCRMSE";
  // parentAppkey = "UGKCRMSE";
  // parentAppkey ="SHRPVERP";
  // parentAppkey ="SCGVTRGRN";
  // parentAppkey ="AARTIMA";
  // parentAppkey ="CDETCRMSE";
  // parentAppkey = "KPBMA";
  // parentAppkey = "SMFAVPM";
  // parentAppkey = "SMFAVERP";
  // parentAppkey = "UGKWHM";
  // parentAppkey = "UGKWHMTEST";
  // parentAppkey = "GLOBCRMSE";
  // parentAppkey = "RADHACRMSE";
  // parentAppkey = "RCPLCRMTEST";
  // parentAppkey = "RADHACRMSE";
  // parentAppkey = "L.BAIDVWHM";
  // parentAppkey = "AONECRMSC"
  // parentAppkey = "PUSHPCRMSE";
  parentAppkey = "CPGTRACK";
  // parentAppkey = "METACRMSE";
  // parentAppkey = "VICCOCRMSR_LHS";
  // parentAppkey = "BAIDVWHM";
  // parentAppkey = "MSPNCRMSE";
  // parentAppkey = "MSPVCRMSE";
  // parentAppkey = "CPGME";
  // parentAppkey = "VICCOCRMSR";
  ...
  constructor(public http: HttpClient, public loadingController: LoadingController, private barScanner: BarcodeScanner, private alertController: AlertController, public modalController: ModalController
    ...
    private platform: Platform,private fileOpener: FileOpener,
     private nativeAudio: NativeAudio, private locationAccuracy: LocationAccuracy, private emailComposer: EmailComposer
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.networkStatus) {
        this.check().then(data => {
          ...
          //  (this.urlArr[0] ? this.urlArr[0].split('lhsws')[0] : "") + ", " + (this.urlArr[1] ? this.urlArr[1].split('lhsws')[0] : "") + ", " + (this.urlArr[2] ? this.urlArr[2].split('lhsws')[0] : "") + ", " + (this.urlArr[3] ? this.urlArr[3].split('lhsws')[0] : "") +
  ...
  check() {
    return new Promise((resolve, reject) => {
      ...
      this.http.get(this.urlArr[0] + 'checkServerStatus').subscribe(data => {
        if (status) {
          console.log("sucess" + this.urlArr[0] + 'checkServerStatus');
        ...
        console.log("fail" + this.urlArr[0] + 'checkServerStatus');
        errorCount++;
        if (errorCount == 4) {
          reject();
        }
      ...
      this.http.get(this.urlArr[1] + 'checkServerStatus').subscribe(data => {
        if (status) {
          console.log("sucess" + this.urlArr[1] + 'checkServerStatus');
        ...
        console.log("fail" + this.urlArr[1] + 'checkServerStatus');
        console.log(err);
        errorCount++;
        if (errorCount == 4) {
          reject();
      ...
      this.http.get(this.urlArr[2] + 'checkServerStatus').subscribe(data => {
        if (status) {
          console.log("sucess" + this.urlArr[2] + 'checkServerStatus');
        ...
        console.log("fail" + this.urlArr[2] + 'checkServerStatus');
        console.log(err);
        errorCount++;
        if (errorCount == 4) {
          reject();
      ...
      this.http.get(this.urlArr[3] + 'checkServerStatus').subscribe(data => {
        if (status) {
          console.log("sucess" + this.urlArr[3] + 'checkServerStatus');
        ...
        console.log("fail" + this.urlArr[3] + 'checkServerStatus');
        console.log(err);
        errorCount++;
        if (errorCount == 4) {
          reject();
  ...
  geoLocation() {
    ...
    if (!this.subscribe) {
      this.subscribe = true;
      this.geolocation.watchPosition(options).subscribe((resp: any) => {
        if (resp && resp.coords) {
          this.latitude = resp.coords.latitude;
          this.longitude = resp.coords.longitude;
        }
  ...
  onBlurEvent(item, formulaStr: any, frame: any) {
    try {
      ...
      for (let f of formulaArr) {
        if (f) {
          ...
          if (resultArr[1].indexOf("?") > -1) {
            ...
            let str = conditionsArr[0]
            if ((conditionsArr[0].indexOf('SYSDATETIME') > -1)) {
              let date = new Date();
              str = str.replaceAll("SYSDATETIME", date.getTime());
            }

            if ((conditionsArr[0].indexOf('SYSDATE') > -1)) {
              let date = new Date(new Date().setHours(0, 0, 0, 0));
              str = str.replaceAll("SYSDATE", date.getTime());
            }
            conditionsArr[0] = str;
            ...
            // if ((formula.indexOf('SYSDATETIME') > -1)) {
            //   let date = new Date();
            //   formula[formula.indexOf('SYSDATETIME')] = date.getTime();
            // }
      ...
      if ((item.isValid != undefined && !item.isValid) || alertCond) {
        return 'error';
      }
      return null;
    } catch (err) {
  ...
  downloadPdf2(shareType, docDefinition, title) {
    let directoryPath = "";
    if (this.plt.is("ios")) {
      directoryPath = this.file.documentsDirectory+'NoCloud/';
      // alert(directoryPath);
      ...
      if (this.plt.is("android") || this.plt.is("ios")) {
        ...
        pdfMake.createPdf(docDefinition).getBuffer((buffer => {
          ...
          this.file.writeFile(directoryPath, fileName, blob, { replace: true })
            .then(fileEntry => {
              // alert("directoryPath"+directoryPath)
              ...
              if (fileEntry) {
                ...
                if (fileEntry.isFile) {
                  ...
                  this.fileOpener
                  .open(directoryPath+fileName , 'application/pdf')
                  .catch(error => console.log('files2'));
                }
                ...
                this.file.checkFile(directoryPath, fileName).then((exist => {
                  ...
                  if (this.plt.is("ios")) {
                    ...
                    //   .catch(e => {
                    //     // alert("from fileOpener");
                    //     this.presentAlert("file is" + JSON.stringify(e))
                    // });
                  }
                })
                ).catch(err => {
                  // alert("from checkFile");
                  this.presentAlert('Directory' + JSON.stringify(err));
                });
              }
            })
            .catch(err => {
            //  alert("from write file");
  ...
  //             this.displayCordovaToast("File Generated");
  //             this.fileOpener
  //                 .open(directoryPath+fileName , 'application/xlsx')
  //                 .catch(error => console.log('files2'));
  //           }
      ...
      this.androidPermissions.hasPermission(this.androidPermissions.PERMISSION.READ_EXTERNAL_STORAGE)
        .then(status => {
          this.androidPermissions.hasPermission(this.androidPermissions.PERMISSION.WRITE_EXTERNAL_STORAGE).then((writeStatus) => {
            if (writeStatus.hasPermission) {
              if (status.hasPermission) {
                this.file.checkDir(this.file.externalRootDirectory, "Download").then((res) => {
                  resolve("success")
                }, (error) => {
  ...
  setItemDefaultValue(item, parentComponent) {
    if (item.item_default_value) {
      ...
      if (item.item_default_value == "USER_CODE") {
        ...
        item.value = this.latitude;
      } else if (item.item_default_value == "LONGITUDE") {
        item.value = this.longitude;
      } else if (item.item_default_value == "LOCATION") {

        this.geoCoderLocation(this.latitude, this.longitude).then(res => {
          item.value = res;
        }, (error) => {
          item.value = "";
          // alert(JSON.stringify(error))
