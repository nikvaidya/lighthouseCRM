...
import { Platform, NavController, IonRouterOutlet, AlertController, ModalController, Events, MenuController, TextValueAccessor, PopoverController } from '@ionic/angular';
...
export class AppComponent implements OnInit{
  ...
  initializeApp() {
    ...
    this.platform.ready().then(() => {
      ...
      if (this.platform.is('android') || this.platform.is('ios')) {
        alert("plugin come inside platforms")
        // this.fcm.subscribeToTopic('all');
        // this.fcm.getToken().then(token => {
        //   if (token) {
        //     alert("tolen generated"+token)
        ...
        this.fcm.getInitialPushPayload().then((payload) => {
          alert("getInitialPushPayload run ")
        ...
        //   alert('push permissions recieved');
        // }, (e) => {
        //   alert('push permissions failed', e);
        // });
        ...
        // alert("firebaseX plugin start in ios ")
        ...
        //     if (data.wasTapped) {
        //       alert("user open a notifications"+data);
        //       this.openPage();
        //     } else {
        //       this.openPage();
        //     }
  ...
  logout() {
    ...
    this.globalObjects.destroyLocalData("GOOGLE_MAP_KEY");
  ...
  checkForUpdate() {
    this.market.open('com.lhs.lhsapperp2');
  }
  ...
  switchToCollection() {
    this.globalObjects.switchapp = true;
  ...
  // this.globalObjects.inappBrowser("https://play.google.com/store/apps/details?id=MS Life Bandhan 600+.sangitacrm.keprocrmskrcrm&hl=en&pli=1")
  ...
  //         // alert('Permission is not granted, request it')
